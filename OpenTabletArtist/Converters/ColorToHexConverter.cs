...
/// the color picker's inline hex field (#622). Alpha is dropped (the pickers are opaque-only); a half-typed
/// or invalid string leaves the color untouched rather than clobbering it.</summary>
public sealed class ColorToHexConverter : IValueConverter
{
    ...
    public object? ConvertBack(object? value, Type targetType, object? parameter, CultureInfo culture)
        => value is string s && Color.TryParse(s, out var c)
            ...
            : BindingOperations.DoNothing;    // keep the current color while the text is incomplete/invalid
}
