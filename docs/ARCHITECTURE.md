...
- `Controls/` — custom controls: `PenTestCanvas` (Scribble — SkiaSharp paint surface), `PressureCurveChart` (Dynamics curve editor), `ScreenMappingDiagram` (unified to-scale monitor picker + tablet active-area + live pen dot + gradient side-beam connector), `TabletStatusBanner` (detected/connected banner), `SakuraPetals` (falling-petal overlay)
