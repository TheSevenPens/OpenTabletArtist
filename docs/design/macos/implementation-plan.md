...
- **Every Phase 0 and most later PRs are Windows-behaviour-preserving.** The one deliberate Windows change is
  the output-mode generalisation (Phase 3), and it only affects an uncommon config (a Windows tablet on OTD's
  *native* absolute mode) — an improvement, not a regression. See the hub's merge-safety analysis.
...
**Goal:** make `master` port-ready and cleaner, with **zero** macOS code and **zero** Windows behaviour change.
Each item is an independent, mergeable PR. After Phase 0, the later phases are small and low-risk.

| # | PR | What | Why it's a general win | Windows-safe? |
|---|---|---|---|---|
| 0.1 | **Enable CA1416 + annotate** | Turn on the platform-compatibility analyzer; add `[SupportedOSPlatform("windows")]` to the Windows-only classes (see the [surface catalog](windows-specific-surface.md)) | Unguarded Win32 calls become **compile-time** errors; documents intent | ✅ analyzer-only + attributes |
| 0.2 | **Extract OS seams (Windows impls only)** | Introduce `IDisplayEnumerator`, `IGlobalHotkeys`, `IStartupManager`, `ISingleInstance`, `IElevation`; move existing Win32 code behind them; static facade + `.Use()` for tests | Untestable static P/Invoke becomes unit-testable; one factory owns the platform choice | ✅ behaviour-preserving refactor |
| 0.3 | **Generalise domain conflations** | Output mode: detect Absolute/Relative by mode path (native + WinInk), platform-preferred selection; Health: pass an `IsWindows` input flag (default `true`) instead of inline checks | Cleaner domain modelling; pure, testable evaluator; fixes a mislabeled toggle for native-absolute tablets on Windows too | ✅ identical for the common WinInk case |
| 0.4 | **Kill hardcoded platform strings** | Platform-aware `DaemonExeName`; audit for `.exe` / `\` / `C:\` / registry literals | No magic strings encoding a platform; prevents the class of bug that broke Restart | ✅ `.exe` on Windows |
| 0.5 | **Test + CI hygiene** | Make path-dependent tests build OS-appropriate paths (`Path.Combine` + per-OS root); add a **Linux `dotnet build` + `dotnet test` CI lane** | Portable suite; catches "compiles/passes only on Windows" regressions immediately | ✅ Windows-identical paths |
| 0.6 | **Defensive fallbacks + explicit guards** | Add graceful fallbacks to single-point-of-failure OS calls (daemon discovery/version); replace swallow-everything `try/catch` (e.g. `GlobalHotkeyService` ctor, `ProfileToast`) with explicit `IsWindows()` guards | Reliability on Windows too (elevated processes, AV, etc.); predictable failure | ✅ same result on Windows |

**Exit criteria:** `master` builds + all tests pass on Windows **and** on the new Linux CI lane; the OS
seams exist as interfaces with Windows impls; CA1416 is green; no hardcoded platform strings remain.
...
- Gate the **health catalog** off-Windows (WinInk plugin, VMulti, driver-conflict, "running as admin"),
  keeping the cross-platform checks (display mapping, pen dynamics, config override, external daemon).
...
- Fix the **calibration overlay coverage** on macOS: `WindowState.FullScreen` only covers the working area and
  ...
  full `NSScreen` frame. Keep the calibration overlay's user32 calls (`SetCursor`, `Win32Properties` hook)
  guarded (Phase 0.6 / 5).
...
- **Guarding (required):** ensure every OS seam no-ops/degrades off-Windows — global hotkeys, tray (already
  cross-platform via Avalonia `TrayIcon` — a menu-bar item on macOS), foreground watcher, single-instance,
  elevation, toast, shell-feedback, clipboard, the calibration overlay's user32 calls.
...
workflow, independent of the app itself. Everything upstream is proven.
...
1. CA1416 + `[SupportedOSPlatform]` annotations.
2. Extract `IDisplayEnumerator` (+ facade) — Windows impl only.
3. Extract remaining OS seams (`IGlobalHotkeys`, `IStartupManager`, `ISingleInstance`, `IElevation`).
4. Generalise output-mode detection; make `HealthEvaluator` take `IsWindows`.
