...
- **We already implement the snapshot UX.** Our **Saved Settings** page (`PresetsViewModel`) saves,
  ...
  `AppSession.ApplyAndSaveSettingsAsync(Settings)` ([AppSession.cs:427](../../OpenTabletArtist/Services/AppSession.cs))
...
2. **Our app must be running.** OTD won't switch on its own; this only works while OpenTabletArtist is
...
3. **Persistence collision (concrete).** `ApplyAndSaveSettingsAsync` **always** persists via `TrySave`
   ([AppSession.cs:435](../../OpenTabletArtist/Services/AppSession.cs)), so reusing it for every focus
