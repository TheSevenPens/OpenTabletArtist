...
- **Your tablets** — every paired or currently-connected tablet as a card: the tablet name with a **detection icon** beside it (a green link when it's detected, a muted link-off when it isn't — the same icon the tablet switcher uses, with "Detected" / "Not detected" as its tooltip), the "last seen" time for a remembered one, and a **Settings** button (a pencil icon) that opens its Tablet page plus a **Forget** button. An explicit "No tablets connected or remembered" state shows when there are none.
...
The **Tablet** page's tabs are **about**, **mapping**, **calibration**, **buttons**, and **wheels** (plus **hover**, **filters**, and **json**, hidden unless enabled on **Settings → Developer**). *(Pen dynamics — now the **pressure** tab — moved to the **Pen** page; see below.)* Each tab is its own section below.
...
The **Pen** page's tabs are **movement**, **inputs**, and **pressure** — each its own section below. It shares the Tablet page's header (linked switcher + Refresh).
