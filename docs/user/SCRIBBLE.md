...
The scribble page has several brush types to explore different facets of the pen, from pressure to tilt and barrel rotation; pick one from the **Brush** dropdown at the left of the controls row. At the far right of the controls row is an **App / Driver** toggle that picks which data the scribble page draws with: **Driver** (the default) uses the raw pen data the driver reports, which works even before Windows Ink is fully set up; **App** uses the pen data that reaches an application through Windows Ink. This is useful for debugging whether an issue lies with the tablet and the driver versus Windows Ink.
