...
- **Gradients** — a live editor for the glows in Sakura's CodeGen backdrop. Only visible in effect while that background is selected. Every glow is listed on the left with its colour and what it is; picking one opens it on the right, under a preview of the glow itself. A glow is either **radial** (a soft blob placed along its edge, with **Center X** and **Width**) or **linear** (a wash spanning the whole edge, with a **Falloff** for the shape of its fade); **Edge** anchors it to the bottom, top, left or right of the window, and **Reach** is how far it comes in from there. Radial glows stay on the bottom and top edges. **Copy settings JSON** puts the whole backdrop on the clipboard.
