...
This section controls how pressure works. Here, you can control the pressure curve by dragging the node in the middle of the curve up or down, making the pen either more or less sensitive. Pressure smoothing allows you to get rid of jitter that is often present when you're drawing with very low pressure. The tab is three columns: the curve and its sliders, then a small canvas where you can draw and see what these settings actually do with pressure, then the live readouts — **raw** as it comes back from the pen, **processed** after the curve and smoothing — with a gauge showing both. So you can see how smoothing and the curve actually affect the data visually.
