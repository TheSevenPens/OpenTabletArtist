...
- `Views/` — Avalonia AXAML pages (Home, Tablets, Pen, Saved Settings, Custom Tablet Compatibility, Driver Cleanup, Diagnostics, Log, Plugins, Scribble, Daemon, Theme, About) and the per-tablet `TabletDetailView` — shown **in-app** for the Tablets nav (#tablet-ux-overhaul), and also hosted by the tray's thin `TabletSettingsDialog` for the focused Pen Dynamics editor. Pen **dynamics** (pressure/position/tilt) is a shared `DynamicsView` control hosted by the PEN page's dynamics pivot and by that focused editor (#pen-dynamics-move). The SETTINGS *System* pivot is a two-column `SystemSettingsView` (Startup + Shortcut left, Driver Cleanup right); the tablet switcher cluster is the shared `TabletSwitcherBar`
...
- `Controls/` — custom controls: `PenTestCanvas` (Scribble — SkiaSharp paint surface), `PressureCurveChart` (Dynamics curve editor), `PressureCurveThumbnail` (non-interactive mini curve preview for the Dynamics preset picker — reuses the shared `PressureCurve.Apply` math), `ScreenMappingDiagram` (unified to-scale monitor picker + tablet active-area + live pen dot + gradient side-beam connector), `TabletStatusBanner` (detected/connected banner), `SakuraPetals` (falling-petal overlay)
...
| `IDeviceData` | Tablet/device data produced by the data load, plus the `DataLoaded` event — including the full connected-tablet set (`DetectedTablets`, one Dashboard card each) and the user-selectable **active tablet** (`ActiveTabletName`/`SetActiveTablet`) that the single-target flows (tray actions, Test, Diagnostics) act on (#190). The **Tablet, Pen, and Scribble tablet switchers are all linked to this active tablet** — selecting on one updates the others (the shell mirrors `ActiveTabletName` changes into the Tablet/Pen page switchers; Scribble binds it directly) |
| `IDaemonDebugSession` | The live debug-report subscription used by Diagnostics |
| `IDaemonLogSource` | The daemon log stream, for the Console page and the conflicting-driver monitor |
| `IDaemonPluginService` | Install / uninstall / reload daemon plugins |
| `IDaemonTransport` | Everything `AppSession` needs from the connection (composes the three above). `AppSession` takes this rather than a concrete `DaemonClient`, so the data load and the apply path are testable against a fake — see `FakeDaemonTransport` (#740) |

The settings themselves — the current `Settings`, the load/persist revision baselines, the pending
unsaved change, the per-app override flag and the apply-loop breaker — live in
`Services/SettingsCoordinator.cs` (#740). `AppSession` keeps the `ISettingsCoordinator` contract and
delegates, so no consumer sees the split; it also keeps the apply→reload orchestration, so the
coordinator holds no reference back to the session and stays headless and thread-agnostic. That is what
makes the apply, persist, restore and override paths testable without a dispatcher.
...
**Quitting is a sequence, and the order is load-bearing (#828).** `QuitSequence` (`OpenTabletArtist/Services/QuitSequence.cs`) holds it, lifted out of `AppTray` so it can be tested — the tray needs a real `TrayIcon` and a desktop lifetime, so none of it was reachable before. Restore the per-app default (#167), then stop the daemon if asked (#596), then settle and close the settings session (#828). Each step needs the connection the next one takes away, and running them out of order fails silently rather than loudly: the step simply does nothing. Every step is bounded by `QuitSequence.StepBound` (5s) so a daemon that has stopped answering cannot hold an exit open — the application's own choice, not the library's ten-second default.

The window's `Closed` handler still runs `MainViewModel.Dispose()`, which **cannot** wait, and that is deliberate: it is the path taken when something other than Quit ends the process. `AppSession.CloseAsync` is the one that settles; `Dispose` remains the teardown and the fallback, and is idempotent because the two now run in sequence.
...
> ### ⚠️ HiDPI pixel-perfect rendering — get this exactly right (a very common mistake)
>
> When you hand-render into a bitmap and blit it to the screen (SkiaSharp, `WriteableBitmap`, `RenderTargetBitmap`, a canvas control — any framework), the backing bitmap **must be sized in physical device pixels, not logical/DIP units**, or every stroke is drawn at a lower resolution and then upscaled by the compositor → soft, rough edges. This is the single most common thing AI-generated (and hand-written) drawing surfaces get wrong, because a control's `Bounds` are in **logical** units and "it looks fine at 100% scaling" — the bug only appears on a scaled display (125/150/200%). The correct recipe (see `Controls/PenTestCanvas`):
>
> 1. **Read the render scaling.** `var scale = TopLevel.GetTopLevel(this)?.RenderScaling ?? 1.0;` (1.0 at 100%, 1.5 at 150%, 2.0 at 200%). Re-read it whenever you (re)create the surface — it changes when the window moves between monitors.
> 2. **Size the bitmap in physical pixels:** `w = round(Bounds.Width * scale)`, `h = round(Bounds.Height * scale)`. This is the resolution the display actually has.
> 3. **Keep the bitmap's DPI at 96** (`new WriteableBitmap(new PixelSize(w, h), new Vector(96, 96), …)`). This is the subtle part: DPI 96 means the bitmap's *logical* size equals its *physical* pixel count (`w×h`). In `Render`, `context.DrawImage(bmp, new Rect(Bounds.Size))` then **downscales** those `w×h` physical pixels into the smaller logical bounds — which, composited back out at `scale`, lands them 1:1 on the device. **Do not** set the DPI to `96×scale`: that makes the bitmap's logical size equal `Bounds`, so `DrawImage` does *no* downscale and the physical pixels get placed 1 bitmap-pixel per **logical** unit — the drawing renders at **2× (at 200%) and offset**, diverging further from the pen the farther it travels. (We hit exactly this; it's an easy trap because the documented "DPI = 96×scale" pattern is for the *opposite* direction — sizing a bitmap you'll draw at logical coordinates.)
> 4. **Draw and hit-test in the right space.** Stamp into the bitmap in **physical** pixels (multiply normalized/logical input coords by `scale`, and scale brush radii/line widths by `scale` so they keep their on-screen size). Anything drawn through the Avalonia `DrawingContext` instead (overlays, a cursor crosshair) stays in **logical** coordinates — don't mix the two.
> 5. **Force nearest-neighbour on the blit** (`RenderOptions.SetBitmapInterpolationMode(this, BitmapInterpolationMode.None)`) so a fractional-bounds sub-pixel mismatch can't reintroduce a smoothing pass over already-antialiased content.
>
> Litmus test: at 150%/200% scaling the strokes are crisp *and* the ink lands exactly under the pen across the **whole** canvas (not just near the top-left origin). If it's sharp near the origin but drifts away from the pen toward the far corners, the bitmap/blit spaces are mismatched — revisit steps 3–4.
...
**Interactive area mapper (#59/#199).** The tablet page's ACTIVE AREA tab is an editable diagram (`Controls/ActiveAreaDiagram`): drag to move, corner-drag to resize (opposite corner anchored, aspect locked to the display), a Size slider, and a Maximize button. Rotation (None/90/180/270) is a selector on the same tab — OTD applies the *tablet* (input) area's `Rotation` about its centre (and `AbsolutePositionMapper` mirrors it), so 180° is a straight flip and 90°/270° resize the area to fit the tablet once rotated (`AreaMappingCalculator.FitForRotation`). The pure geometry — the aspect fit, rotation-aware `ClampArea`, and the mapping — lives in `Domain/` (unit-tested); the diagram does the screen↔tablet coordinate transforms (rotation-aware) and raises committed edits to `TabletDetailViewModel`, which persists via the normal apply path. Pen-display portrait (rotate the panel + the Windows display) is tracked in #490.
...
- **Corners** (default) → a **least-squares affine** (`CalibrationSolver.Solve`, 6 DOF over 4 corners), correcting offset/scale/rotation/shear. Because it's over-determined it averages tap noise instead of fitting it, so the correction stays smooth. (A #195 experiment made corners an exact **perspective homography** — `CalibrationMath.SolveHomography` — but from exactly 4 taps it overfit their noise and warped the corner neighbourhoods, so corners reverted to affine in #483; the homography solver is kept for reading legacy stores.)
...
`CalibrationSolver` turns measured taps + `AbsolutePositionMapper.MapFromDesktop` into the chosen model. The correction is applied by a second plugin filter (`OpenTabletArtist.Dynamics.CalibrationFilter`, `PreTransform`, before the dynamics filter), which branches on a stored `Model` field (affine / grid / legacy homography). It's persisted via `Services/CalibrationProfile` (mirrors `PressureCurveProfile`; tags the model + payload, keeps a mapping fingerprint for staleness). The UI is a full-display overlay (`Views/CalibrationOverlayWindow` + `ViewModels/CalibrationViewModel`) launched from the Screen-Mapping tab in Absolute mode, with the mode picked from a selector next to **Calibrate…**; capture uses the daemon debug stream and bypasses any existing calibration so taps are recorded uncorrected. See `docs/design/127-pointer-calibration.md`.

**Theming.** Light, Dark, and a custom **Sakura** (`Anime`) variant are implemented via `ResourceDictionary.ThemeDictionaries` in `Themes/Colors.axaml` (theme-overridable keys — accent/glass/backdrop — live **only** in the per-variant dictionaries, since a top-level key would shadow them). `Services/ThemeService` persists the user's choice (`AppSettings`) and applies it through `Application.RequestedThemeVariant` — "System" follows the OS; Sakura defaults on. The selector lives on the **Theme** page (`SettingsViewModel` / `SettingsView`), which also hosts the Sakura falling-petals toggle (`AnimationSettings` + `SakuraPetals`) and the frosted-glass card-opacity slider (`AcrylicSettings`, which drives the global `GlassBgBrush`). Custom-drawn controls still use fixed colors and could be re-tuned per variant. Both blossom backdrops (#556) are code-generated by default — Dark Sakura joined Sakura when its own artwork was retired, since that picture had a glow painted into its bottom edge that no setting could reach. `Services/GradientBackground` builds a flat base color plus a glow brush per window edge — each glow is either a soft **radial** blob centred somewhere along its edge or a **linear** wash spanning it, anchored to one of four fixed-thickness bands in `MainWindow` (bottom, top, left, right; radial is offered on the two horizontal edges only). `GradientBackground.ApplyGlowBrushes` writes all four into `Application.Resources` for both callers, `ThemeViewModel.RefreshSkin` and the live editor. Each skin keeps its own glows and base color under `<skin>:CodeGenGlows` / `<skin>:CodeGenBaseColor`: both are edited on **Settings → Theme → BACKDROP**, where `ThemeViewModel` owns the `GradientEditorViewModel` and the two sit beside a live preview of the pair. The editor re-reads the current skin's set on `ThemeService.SkinChanged`. It was a Developer subtab until #appearance-merge, which put a skin's glows a tab away from the base color they paint over; Theme took the rail Dev already had (theme · backdrop · colors) since the three no longer fit one screen. A flat **solid color** mode remains; the old cherry-blossom **image** mode was retired and migrates to codegen.

**App icon / branding (#600).** The vector SVGs are the source of truth (`Assets/appicon.svg` full-color, `Assets/appicon-bw.svg` single-color silhouette); the shipped rasters (`appicon.png` → `MainWindow.Icon`, `appicon.ico` → the exe `ApplicationIcon`) are rendered from the color SVG offline (no runtime SVG dependency). The Home page's About column draws the B&W mark from the same geometry as an inline theme-tinted vector (`AboutView.axaml`, `Fill = TextPrimaryBrush`), so the mono mark reads on both light and dark skins. See `Assets/ICON.md` for the file roles + regen steps.
...
**Releases.** `.github/workflows/release.yml` publishes a downloadable Windows build when a `v*` tag is pushed (or via manual dispatch). It runs the tests, `dotnet publish`es the app **self-contained for `win-x64`** (no .NET runtime needed on the user's machine) and the OTD daemon into a `Daemon/` subfolder next to the app, **strips every `.pdb`** from the bundle, zips the result, and attaches it to a GitHub Release. The **release notes are built from the commit messages since the previous tag** (each commit's subject → a heading, its body verbatim, minus the `Co-Authored-By` trailer and the version-bump commit), so descriptive commit messages carry straight into the changelog — write them accordingly (it falls back to GitHub's auto-generated notes only if there's no prior tag). The checkout uses `fetch-depth: 0` so that diff is available. The bundled `Daemon/` path is what `Domain/DaemonExePaths` checks first, so the daemon auto-starts from the release layout exactly as in dev. To cut a release: bump `<Version>` in the csproj, then `git tag v0.1.0 && git push origin v0.1.0`.

**Release size.** The zip carries **two self-contained .NET runtimes** — the app (net10) and the bundled daemon (net8) — which is the bulk of its size (~87 MB), the deliberate cost of a zero-install download. (OTD's own Windows builds are much smaller because they're *framework-dependent* — the user must have .NET installed.) Debug symbols are **not** shipped: the strip step removes ~100 MB uncompressed, almost all of it the native `libSkiaSharp.pdb` (~80 MB) + `libHarfBuzzSharp.pdb` (~20 MB) that the SkiaSharp/HarfBuzz NuGet packages drop into the output — `-p:DebugType=none` only covers *managed* symbols, so the native ones have to be deleted as files. The next lever is single-file publishing (`PublishSingleFile` + compression): it would collapse the ~250 loose files in the zip root down to one `OpenTabletArtist.exe` and shrink the payload further (verified working; tracked in #585/#586).

**The OTD submodule.** OpenTabletDriver is a git submodule pinned to a **vetted release tag** (not a branch). `.github/workflows/otd-release-watch.yml` runs weekly (and on demand): it compares OTD's latest release tag to our pin and, when OTD ships a newer release, opens a PR bumping the submodule to it — never touching `master` or auto-merging, so CI + a human gate every bump. Dependabot's `gitsubmodule` support was **not** used: it tracks a branch's latest commit rather than release tags, which would pull us onto unreleased commits (#182/#587).
