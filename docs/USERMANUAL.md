...
- **Supported tablets** — **View list** opens an in-app dialog listing every tablet OpenTabletDriver supports (read from the bundled daemon, offline). It's searchable, shows a **X of 339** count as you filter, numbers each row with its active area / pressure levels / button count, and highlights (and scrolls to) your connected tablet if it's in the list.
