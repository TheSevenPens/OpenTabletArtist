...
- **Pen Inputs** — The pen's **tip** and **eraser**, laid out under a diagram of the pen (which sits at the top of the page). Each shows a green check + "Adaptive Binding (recommended)" when already on Adaptive Binding; otherwise a **Fix** button sets it. A **Pen input** card holds **Disable pen tip** (#493 — clears the tip binding so tapping does nothing; your previous tip binding is stashed and restored when you turn it back off). *(Binary pressure moved to Pressure Dynamics and Disable tilt to Tilt Dynamics; #494.)*
- **Pen Buttons** — The pen's own barrel buttons (#494), each an editable binding with the same Adaptive/Fix affordance, over a pen diagram that highlights the buttons this pen has. The tab is hidden for pens with no barrel buttons.
