...
- **Pen Dynamics** — An interactive pressure-curve editor **plus smoothing**, enforced by the bundled *OpenTabletArtist – Pen Dynamics* filter so it affects **every** app (Krita, Clip Studio Paint, Photoshop, …), not just one. There's **no on/off switch** — pen dynamics simply do nothing until you shape the curve or raise a smoothing slider (a linear curve with zero smoothing is a true no-op), and the filter stays enabled from then on. A status line tells you whether the current settings are actually affecting the pen, and **Reset all** returns everything to that no-op (this is how you "turn it off").
  ...
  - **Reset all** (in the tab header) returns the curve, both smoothing amounts, and the order to their no-op defaults in one click — the way to neutralize dynamics now that there's no on/off toggle. The curve's own **Reset** button only resets the curve.
