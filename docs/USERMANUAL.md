...
- **Pen Inputs** — All the switches on the pen — tip, eraser, and barrel buttons — laid out over a diagram of the pen. Each shows a green check + "Adaptive Binding (recommended)" when already on Adaptive Binding; otherwise a Fix (tip/eraser) or Fix All (pen buttons) button sets it. A **Pen input** card (#469) adds two toggles: **Disable pressure** (apps get a flat on/off contact — this overrides the Pressure Dynamics curve, which the Pressure Dynamics tab then flags as inert) and **Disable tilt** (apps receive no tilt from this tablet).
