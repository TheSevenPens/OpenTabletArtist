...
- **Tablet** — **One card per connected tablet** (so a second tablet is never hidden behind the first), each showing the tablet name, a one-line spec summary (area · pressure levels · buttons), and a **Settings** button that opens that tablet's settings dialog. When nothing is connected, a single card shows "No tablet detected." Updates automatically via polling.
