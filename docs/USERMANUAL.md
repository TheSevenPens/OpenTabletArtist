...
- **Pen Tip & Eraser** — Current tip and eraser bindings. A green check + "Adaptive Binding (recommended)" shows when each is already on Adaptive Binding; otherwise a Fix button sets it.
- **Pen Buttons** — Pen and auxiliary button bindings. A green check shows when all pen buttons use Adaptive Binding; otherwise a Fix All button sets them.
