...
- **Pressure** — An interactive pressure-curve editor **plus smoothing**. Toggle it on to apply custom pen dynamics to this tablet's profile; they're enforced by the bundled *OTD Windows Helper – Pen Dynamics* filter, so they affect **every** app (Krita, Clip Studio Paint, Photoshop, …), not just one.
  - **Curve** — drag the pink **min** node and cyan **max** node to set where pressure starts and saturates (input → output), use the **Softness** slider to bend the response (positive = lighter/concave, negative = firmer/convex), and tick **Cut below input minimum** to turn the lead-in into a dead zone instead of a pressure floor. **Reset** restores the identity curve.
  - **Smoothing (jitter reduction)** — **Position** smoothing steadies wobbly lines and **Pressure** smoothing evens out pressure jitter (each 0–0.99; higher = steadier but more lag). **Order** chooses whether smoothing runs after the curve (*Curve → Smooth*, default) or before it. Smoothing state resets each time the pen lifts, so strokes start crisp.
