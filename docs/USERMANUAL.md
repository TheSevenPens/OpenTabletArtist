...
- **Pen Inputs** — All the switches on the pen — tip, eraser, and barrel buttons — laid out over a diagram of the pen. Each shows a green check + "Adaptive Binding (recommended)" when already on Adaptive Binding; otherwise a Fix (tip/eraser) or Fix All (pen buttons) button sets it. A **Pen input** card (#469) adds three toggles: **Binary pressure** (apps get a flat on/off contact — this overrides the Pressure Dynamics curve, which the Pressure Dynamics tab then flags as inert), **Disable tilt** (apps receive no tilt from this tablet), and **Disable pen tip** (#493 — clears the tip binding so tapping does nothing; your previous tip binding is stashed and restored when you turn it back off).
